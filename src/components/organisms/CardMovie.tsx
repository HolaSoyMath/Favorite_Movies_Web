...
export function CardMovie({ movie, interaction, className, ...props}: CardMovieProps) {
    ...
    return(
        <div id={idMovie.toString()} className={cn("bg-[#232E46] rounded-xl relative cursor-pointer transition-transform duration-300 ease-in-out hover:scale-105" ,className)} {...props}>
            <div className="relative h-3/5 flex-row">
                <img className="absolute top-0 left-0 w-full h-full object-cover object-bottom z-0 rounded-t-xl" src={`https://image.tmdb.org/t/p/w780/${poster}`} alt={`Pôster filme ${title}`} />
